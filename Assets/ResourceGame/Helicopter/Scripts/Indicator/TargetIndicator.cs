...
public class TargetIndicator : MonoBehaviour
{
    ...
    public void InitialiseTargetIndicator(Health target, Camera mainCamera, RectTransform canvas)
    {
        this.target = target;
    ...
    public void UpdateColor(float distance,TypeObjetive type)
    {
        if (target == null) return;
        if(target.TypeObjetive !=type) return;

        if (distance < 50)
        {
            TargetIndicatorImage.color = Color.Lerp(TargetIndicatorImage.color, Color.red, Time.deltaTime * 3.5f);
            _StateIndicator = StateIndicator.InShoot;
        }
        else
        {
            _StateIndicator = StateIndicator.OutShoot;
            TargetIndicatorImage.color = Color.Lerp(TargetIndicatorImage.color, Color.green, Time.deltaTime * 15f);
        }
